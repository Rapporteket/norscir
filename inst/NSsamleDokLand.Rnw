...
<<'LasteDataDb', include=FALSE, echo=FALSE, eval=T>>=
...
query <- "SELECT
	 ...
	 norScir.`PatientInRegistryKey`+'0' AS PatientInRegistryKey
FROM
     `norScir` norScir"
...
VOTabell <- xtable(VerdiOmr, digits=0, align=c('l', rep('r', 5)), 
		caption = capVO,
		label = labVO)
print(VOTabell)
...
AndelsTab <- xtable(Andeler, digits=c(0,0,1), align=c('l', 'r', 'r'), 
...
<<FigNevrNivaaInnUtAlle, echo=FALSE, message=FALSE>>=
	FigAndelStabel(NSdata, libkat=libkat, valgtVar='NevrNivaaInnUt', datoFra=datoFra, datoTil=datoTil, 
			minald=minald, maxald=maxald, erMann=erMann, traume=traume, egenavd=0, reshID=reshID, )	#, sml=sml)
@
}
\end{center}
...
<<FigAlder, echo=FALSE, message=FALSE>>=
	FigFordeling(NSdata, libkat=libkat, outfile='', valgtVar='Alder', datoFra=datoFra, datoTil=datoTil, 
			minald=minald, maxald=maxald, traume=traume, egenavd=egenavd, reshID=reshID, sml=sml)
@
}
...
<<FigDagerRehab, echo=FALSE>>=
	FigFordeling(NSdata, libkat=libkat, valgtVar='DagerRehab', datoFra=datoFra, datoTil=datoTil, 
			traume=traume, egenavd=egenavd, reshID=reshID, sml=sml)
@
}
